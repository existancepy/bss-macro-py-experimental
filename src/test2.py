...
class HasteCompensation():
    ...
    def getHaste(self):
        ...
        def match_haste(index_img):
            i, e = index_img
            res = templateMatch(e, screen)
            _, val, _, _ = res
            return (i + 1, val) if val > 0.7 else (0, 0)
    
        with ThreadPoolExecutor() as executor:
            results = executor.map(match_haste, self.hasteStacks)

        for i, val in results:
            if val > bestHasteMaxVal:
                bestHasteMaxVal = val
                bestHaste = i
                if val > 0.95:
                    break  #early exit on very confident match
